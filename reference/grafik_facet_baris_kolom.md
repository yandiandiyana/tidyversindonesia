...
``` r
grafik_facet_baris_kolom(rows = NULL, cols = NULL, ...)
```
...
#> Error in combine_vars(data, params$plot_env, rows, drop = params$drop): At least one layer must contain all faceting variables: `cyl`
#> ✖ Plot is missing `cyl`
